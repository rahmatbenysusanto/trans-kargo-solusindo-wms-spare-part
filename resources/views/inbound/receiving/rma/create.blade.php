...
@section('title', 'Create Receiving RMA')
...
@section('js')
    ...
    <script>
        ...
        function submitPO() {
            ...
            if (!clientId || !receivedDate || !receivedBy || !itsmNumber) {
                Swal.fire('Error', 'Please fill in all required fields (ITSM#, Client, Date, Received By).', 'error');
                return;
            }

            Swal.fire({
                title: 'Are you sure?',
                text: "Do you want to create this Receiving RMA?",
                icon: 'question',
                    ...
                    fetch('{{ route('receiving.store.rma') }}', {
                        ...
                        .then(data => {
                            if (data.status) {
                                ...
                                Swal.fire({
                                    title: 'Success!',
                                    text: 'Your Receiving RMA has been created successfully.',
                                    icon: 'success'
                                }).then(() => {
                                    window.location.href = '{{ route('receiving') }}';
                                });
                            } else {
                                Swal.fire('Error', data.message || 'Failed to create Receiving RMA.', 'error');
                            }
                        })
                        .catch(error => {
        ...
        function editProduct(index) {
            ...
            document.getElementById('oldSnStatus').innerHTML = '';
...
@section('content')
    <div class="row">
        <div class="d-flex justify-content-end mb-3">
            <a class="btn btn-primary btn-sm text-white" onclick="submitPO()">Create RMA</a>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-4 mb-3">
                            <label class="form-label">RMA# (Optional)</label>
